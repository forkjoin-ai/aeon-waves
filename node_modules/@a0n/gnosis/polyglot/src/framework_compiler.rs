...
pub fn compile_framework_to_gg(topology: &FrameworkTopology) -> String {
    ...
    gg.push_str(&format!("// Assumed from: {}\n\n", topology.file_path));
    ...
    if !topology.middleware.is_empty() {
        ...
        for (i, mw) in topology.middleware.iter().enumerate() {
            ...
            gg.push_str(&format!("({})-[:PROCESS]->({})\n", prev_node, mw_node));
            prev_node = mw_node;
        }
        gg.push_str(&format!("({}: MiddlewareComplete)\n", "mw_done"));
        gg.push_str(&format!("({})-[:PROCESS]->(mw_done)\n\n", prev_node));
    }
    ...
    gg.push_str(&format!("({})-[:PROCESS]->(router)\n\n", route_input));
    ...
    for (i, (path, routes)) in route_groups.iter().enumerate() {
        ...
        if routes.len() == 1 {
            ...
            gg.push_str(&format!("({}: BuildResponse)\n", response_node));
            gg.push_str(&format!(
                "({})-[:PROCESS]->({})\n\n",
