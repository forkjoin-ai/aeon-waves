#!/bin/sh
# Gnosis polyglot execution harness for Haskell.
#
# Uses runghc (GHC interpreter) to run the target file.
...
# runghc interprets Haskell source directly.
if command -v runghc >/dev/null 2>&1; then
  RUN_STDOUT=$(runghc "$FILE_PATH" 2>"$TMPDIR/gnode_stderr_$$") || EXIT_CODE=$?
elif command -v ghc >/dev/null 2>&1; then
  BINARY="$TMPDIR/gnode_hs_$$"
  ghc -o "$BINARY" "$FILE_PATH" 2>"$TMPDIR/gnode_stderr_$$" || {
  ...
  rm -f "$BINARY" "${FILE_PATH%.hs}.hi" "${FILE_PATH%.hs}.o"
else
  printf '{"status":"error","value":"ghc/runghc not found","stdout":"","stderr":""}'
  exit 0
fi
