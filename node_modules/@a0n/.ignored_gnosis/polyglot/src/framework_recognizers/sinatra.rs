        ...
        for line in source.lines() {
            ...
            for (method_name, method) in &[
                ...
                if let Some(path) = path {
                    let handler_name = format!("{}_{}", method_name, sanitize_path(&path));
            ...
            // Port: set :port, 4567
            if let Some(stripped) = trimmed.strip_prefix("set :port,") {
                let rest = stripped.trim().trim_start_matches(',').trim();
