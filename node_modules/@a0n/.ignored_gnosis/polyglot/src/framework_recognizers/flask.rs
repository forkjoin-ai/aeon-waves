...
impl FrameworkRecognizer for FlaskRecognizer {
    ...
    fn detect(
        ...
        let has_flask = source.contains("from flask import") || source.contains("import flask");
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if let Some(route_info) = parse_route_decorator(trimmed) {
                ...
                if let Some(func_name) = find_decorated_function(&lines, i + 1) {
                    let handler_cfg_index =
                        cfgs.iter().position(|cfg| cfg.function_name == func_name);
            ...
            for (decorator, method) in &[
                ...
                if let Some(stripped) = trimmed.strip_prefix(decorator) {
                    if let Some(path) = extract_string_arg(stripped) {
                        if let Some(func_name) = find_decorated_function(&lines, i + 1) {
                            let handler_cfg_index =
                                cfgs.iter().position(|cfg| cfg.function_name == func_name);
