#!/bin/sh
# Gnosis polyglot execution harness for Zig.
#
# Uses `zig run` which compiles and runs in one step.
...
if command -v zig >/dev/null 2>&1; then
  RUN_STDOUT=$(zig run "$FILE_PATH" 2>"$TMPDIR/gnode_stderr_$$") || EXIT_CODE=$?
else
  printf '{"status":"error","value":"zig not found","stdout":"","stderr":""}'
  exit 0
fi
